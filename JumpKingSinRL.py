...
class JumpKingSinRL(gym.Env):
	...
	def reset(self, seed=None, options=None):
		...
		self.log_file = open("episode_log.csv", "w", encoding="utf-8")

		self.log_file.write(
			"accion_numero,accion,altura,ganancia,mejor_altura,recompensa,recompensa_total\n"
		)
	...
	def step(self, action):
		...
		f"best={self.best_height:.1f} | "
		f"reward={reward:.3f} | "
		f"total={self.episode_reward:.3f}"
		)

		self.log_file.write(
			f"{self.action_counter},"
			f"{action_names[action]},"
			f"{new_height:.1f},"
			f"{height_gain:.1f},"
			f"{self.best_height:.1f},"
			f"{reward:.3f},"
